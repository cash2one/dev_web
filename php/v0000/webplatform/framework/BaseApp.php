 ...
 *
 * @author      litwrd <litwrd@gmail.com>
 * @copyright   2015 litwrd
 * @link        https://github.com/litwrd/
 * @license     MIT <https://github.com/litwrd>
 * @version     1.0.0
 * @package     C
 */
namespace C;
...
class BaseApp {
    ...
    public function __construct($options = [], $loader = null)
    {
         ...
         * 注册错误或者异常处理函数
         */
        set_error_handler(array('\C\BaseApp', 'handleErrors'));
        set_exception_handler(array('\C\BaseApp', 'handleException'));
        // 注册 shutdown 函数
        register_shutdown_function(array('\C\BaseApp', 'shutdownOrExit'));

        $option = new Option($options);

        // 创建 Middleware 管理
        $this->middleware = new \C\Middleware\Map();
